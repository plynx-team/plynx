    ...
    render() {
        return ( <
            ...
            div > {
                ...
                onDeprecate = {
                    (fileObj) => this.handleDeprecate(fileObj)
                }
                fileObj = { this.state.fileObj }
                hideDeprecate // TODO let the author to deprecate file
                onPreview = {
                    (previewData) => this.handlePreview(previewData)
                }
                />
            } {
                ...
                onParameterChanged = {
                    (value) => this.handleParameterChanged(this.state.nodeId, this.state.parameterName, value)
                }
                />
            } {
                ...
                onIndexChanged = {
                    (index) => this.handleIndexLinkChange(index)
                }
                readOnly = {!this.state.editable }
                />
            }
            ...
            { /* Visible and flex layout blocks */ } {
                ...
                onBlockMove = {
                    (nid, pos) => this.onBlockMove(nid, pos)
                }
                onNewConnector = {
                    (n1, o, n2, i) => this.onNewConnector(n1, o, n2, i)
                }
                onRemoveConnector = {
                    (connector) => this.onRemoveConnector(connector)
                }
                onOutputClick = {
                    (nid, outputIndex) => this.onOutputClick(nid, outputIndex)
                }
                onSpecialParameterClick = {
                    (nid, specialParameterIndex) => this.onSpecialParameterClick(nid, specialParameterIndex)
                }
                onRemoveBlock = {
                    (nid) => this.onRemoveBlock(nid)
                }
                onCopyBlock = {
                    (copyList, offset) => this.onCopyBlock(copyList, offset)
                }
                onPasteBlock = {
                    (offset) => this.onPasteBlock(offset)
                }
                onBlocksSelect = {
                    (nids) => {
                ...
                onDrop = {
                    (nodeObj) => this.handleDrop(nodeObj, true)
                }
                onAllBlocksDeselect = {
                    () => this.handleBlocksSelect([])
                }
                onSavePressed = {
                    () => this.handleSave()
                }
...

                {
                    ...
                    onParameterChanged = {
                        (node_ids, name, value) => this.handleParameterChanged(node_ids, name, value)
                    }
                    editable = { this.state.editable }
                    ...
                    onFileShow = {
                        (nid) => this.handleShowFile(nid)
                    }
                    onLinkClick = {
                        (node_ids, name) => this.handleLinkClick(node_ids, name)
                    }
                    />
                } <
                /HotKeys>
